...
with Entrez.esearch(db="nucleotide", term="SARS-CoV-2[ORGN]", idtype="acc", retmax="10") as handle:
    results = Entrez.read(handle)

accs = results["IdList"] #save the list of accession numbers    
    
    
#Use efetch to download records and save to diskfilename = "./data/raw/SARS-CoV-2.gbk"
if not os.path.isfile("./SARS-CoV-2.gbk"):
    # Downloading...
    with Entrez.efetch(db="nucleotide", id=accs, rettype="gb", retmode="text") as net_handle: 
        with open('SARS-CoV-2.gbk', "w+") as out_handle:
