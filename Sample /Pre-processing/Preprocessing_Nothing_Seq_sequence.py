...
# in a list of dictionnary/windows. If the total duration of the activitiy in the 
...
# create a list of activities indpendent of their duration !!
    
    for i in range (len(df)) :
        if (i-seq_lenth) < -1 or (i+1) >= len(df) :  
            continue

        else : 
            for ii in range(seq_lenth) :
                    list_activity = list_activity + str(df[i-ii,1])    
...
del(i,dataframe,df,list_activity,list_activity_final)
...
df_words.to_csv('./SPLIT_SEQ_10activities.csv',header = ["Activities","Day"])            
del(df_words) 
